...
function MusicCard({ track, index = 0 }: MusicCardProps) {
  ...
  const renderThumbnail = useCallback(
    (track: Track, imageState: ImageState) => {
      ...
      const badges = [];
      if (track.year) badges.push(<YearBadge year={track.year} />);
      if (spotifyUrl) badges.push(<SpotifyBadge url={spotifyUrl} />);
  ...
  const renderDetailContent = useCallback(
    (track: Track) => <MusicDetailContent track={track} spotifyUrl={spotifyUrl} />,
    [spotifyUrl]
  );
